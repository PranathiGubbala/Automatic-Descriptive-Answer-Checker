...
class TestDAO(object):
    ...
    def saveTestDetails(self,testNo,examTitle,subj,conductedOn,startTime,endTime,noq,marks,teacher_id,subId):
        ...
        sql = "INSERT INTO test(testNo,examTitle,subj,conductedOn,startTime,endTime,noq,marks,teacher_id,subId) VALUES(?,?,?,?,?,?,?,?,?,?)";
        print("okay")
        values = [testNo,examTitle,subj,conductedOn,startTime,endTime,noq,marks,teacher_id,subId];
        print("gng right")
    ...
    def getTestNo(self,student_id,present_date,present_time):
        sql = "SELECT testNo,examTitle,subj,conductedOn,startTime,endTime from test INNER JOIN registerUnder ON test.subId = registerUnder.subId  where registerUnder.student_id = ? and (conductedOn = ? and startTime <= ? and endtime > ?) order by conductedOn,startTime ASC";
        values = [student_id,present_date, present_time, present_time]
