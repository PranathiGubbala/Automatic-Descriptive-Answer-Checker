...
def test():
    ...
    if 'name' in session:
            name = session.get('name')
            name = session['name']
    return render_template("Tests.html",len1 = len(lst),lst=lst,name=name,pd = present_date,pt = present_time)
...
def getAnswerScore(ans, n):
    ...
    teacher_ans = alst[n][0] 
    student_ans = ans
    fw = FuzzyWuzzy()
    var = fw.getMatchPercentage(teacher_ans,student_ans)
...
def studentTestAnswers(testNo,student_id):
    ...
    res = qdao.finalQuestionAnswers(testNo)
    std = StudentAnswer()
    testScore = std.getStdIndiScore(student_id, testNo)
    if 'name' in session:
            name = session.get('name')
            name = session['name']
    #return render_template("CompareAnswers.html",lst=std_answrs,len1=len(std_answrs),name=name)
    return render_template("StudentTestAnswers.html",lst=std_answrs,res=res,len1=len(std_answrs),testScore=testScore,name=name)
